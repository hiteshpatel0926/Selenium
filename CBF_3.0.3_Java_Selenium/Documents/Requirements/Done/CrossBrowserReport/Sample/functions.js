...
function summarize(summary, isStart, result, ts) {
  ...
  if (isStart)
    ...
    times[finish] = ts; // always overwrite
}

function addLog(ts, resId, tcId, tcName, os, browser, eventType, detail) {
 ...
 // var isStart = 
  ...
  var passed = (isStart? null: detail.match(/passed|warning/i));


  // grand summary
  summarize(summary, isStart, passed, ts);
 ...
 db = logs[tcName] = (logs[tcName] ||{
  ...
  summarize(db.summary, isStart, passed, ts);
...
<tr>
<td class='totalText'>Total: ${summary["counts"].total}</td>
